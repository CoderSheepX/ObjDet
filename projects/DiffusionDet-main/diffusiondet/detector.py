...
class DiffusionDet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # Build Backbone.   NAME: "build_resnet_fpn_backbone"
        self.backbone = build_backbone(cfg)
        self.size_divisibility = self.backbone.size_divisibility

        # build diffusion  初始化了模型中的一些参数和缓冲区
        timesteps = 1000
        ...
        # calculations for diffusion q(x_t | x_{t-1}) and others  保存计算扩散的各个中间计算结果的缓冲区
        ...
        # Build Criterion.  建立检测的评估标准 包括分类损失、L1损失、GIoU损失
...
#推理过程 DDIM算法 去噪扩散
    '''每次迭代都是将上次去噪后的结果作为输入到head，进行预测包括噪声，并且删除类别分数小于0.5的框，然后对图片继续DDIM'''
    @torch.no_grad()
    ...
    def forward(self, batched_inputs, do_postprocess=True):
        ...
        # Feature Extraction.  用backbone进行特征提取
        ...
        # Prepare Proposals.
        if not self.training:   #推理阶段 用DDIM算法进行迭代地去噪扩散
            results = self.ddim_sample(batched_inputs, features, images_whwh, images)
            return results

        #训练阶段
        if self.training:
            gt_instances = [x["instances"].to(self.device) for x in batched_inputs]ga
            #对GT逐步加高斯噪声
    ...
    def prepare_targets(self, targets):
        ...
        for targets_per_image in targets:
            ...
            d_boxes, d_noise, d_t = self.prepare_diffusion_concat(gt_boxes)  #核心部分 计算噪声框
